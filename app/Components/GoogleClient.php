...
class GoogleClient
{
    ...
    public function getClient()
    {
        ...
        if (!file_exists(config('googleapi.config.client_path'))) {
            throw new \Exception(
        ...
        $this->client->setAuthConfig(config('googleapi.config.client_path'));
        $this->client->setAccessType('offline');

        $credentialsPath = config('googleapi.config.token_path');
