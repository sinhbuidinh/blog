...
class GoogleGetToken extends Command
{
     ...
     * @var string
     */
    protected $description = 'Get google-client token name';
    ...
    public function handle()
    {
        ...
        $credentialsPath = config('googleapi.config.token_path'); // lấy đường dẫn đến token cũ
    ...
    private function getClient()
    {
        ...
        $client->setApplicationName('KN247'); // đặt tên cho ứng dụng (không quan trọng lắm)
        // cài đặt xin quyền truy cập của token
        $client->setScopes([
            \Google_Service_Drive::DRIVE,
            \Google_Service_Drive::DRIVE_FILE,
            \Google_Service_Drive::DRIVE_METADATA,
        ]);
        $client->setAuthConfig(config('googleapi.config.client_path')); // đường dẫn đến file credentials.json
