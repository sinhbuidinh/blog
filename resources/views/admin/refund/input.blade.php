...
<div class="common_main_wrap">
    <div class="list_wrapper">
        <div class="index_top_block">
            <h1 class="common_page_title">{{ trans('label.refund_parcel') }}</h1>
        </div>
        @include('admin.layouts.session-message')
        <div class="file_form_wrap">
            <form class="login_form skip_alert_changes" action="{{ route('refund.create') }}" id="parcel_form" method="post">
                <div class="hidden">
                    {{ csrf_field() }}
                </div>
                ...
                <div class="form_btn_area center">
                    <button class="btn btn-primary" id="refund_parcel" type="button">{{ trans('label.refund') }}</button>
                </div>
            </form>
        </div>
    </div>
